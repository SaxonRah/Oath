// Oath_RPGSystems.hpp
#ifndef OATH_RPG_SYSTEMS_HPP
#define OATH_RPG_SYSTEMS_HPP
...
public:
    ...
    bool processInput(const std::string& systemName, const TAInput& input = TAInput {});
...
private:
    ...
    void saveQuestJournal(std::ofstream& file, const std::map<std::string, std::string>& journal);
    bool loadQuestJournal(std::ifstream& file, std::map<std::string, std::string>& journal);
    void saveDialogueHistory(std::ofstream& file, const std::map<std::string, std::string>& history);
