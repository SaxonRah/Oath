...
public:
    ...
    bool saveState(const std::string& filename)
    {
        ...
        // Add a verification marker at the end
        const char* endMarker = "VALID_END";
        file.write(endMarker, 9); // Including null terminator
        ...
        if (!verifyFile.is_open()) {
            std::cerr << "Failed to open save file for verification" << std::endl;
            return false;
        }
        ...
        verifyFile.seekg(0, std::ios::beg);

        if (fileSize <= 9) { // Just the marker or less
            std::cerr << "Save file appears to be empty or corrupt" << std::endl;
            return false;
        }

        // Read and display the first few bytes as hex for diagnostics
        ...
        verifyFile.read(reinterpret_cast<char*>(buffer.data()), bytesToRead);
        ...
        verifyFile.seekg(fileSize - 9);

        // Read and verify end marker
        char verifyMarker[10] = { 0 };
        verifyFile.read(verifyMarker, 9);

        if (strcmp(verifyMarker, endMarker) != 0) {
            std::cerr << "Save file verification failed - end marker not found" << std::endl;
            return false;
        }

        std::cout << "Save file successfully written and verified: " << fileSize << " bytes" << std::endl;
        return true;
    }
    ...
    bool loadState(const std::string& filename)
    {
        ...
        if (fileSize <= 9) { // Just the marker or less
            std::cerr << "Save file appears to be empty or corrupt (too small)" << std::endl;
            return false;
        }

        // Check end marker
        file.seekg(fileSize - 9);
        char endMarker[10] = { 0 };
        file.read(endMarker, 9);

        if (strcmp(endMarker, "VALID_END") != 0) {
            std::cerr << "Invalid or corrupted save file (missing end marker)" << std::endl;
            return false;
        }

        // Reset to beginning for normal loading
        file.seekg(0, std::ios::beg);
