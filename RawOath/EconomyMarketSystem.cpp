...
private:
    ...
    void generateStock()
    {
        ...
        // Generate appropriate items based on market type
        switch (type) {
        case MarketType::BLACKSMITH:
            generateBlacksmithItems(inventorySize);
            break;
        case MarketType::ALCHEMIST:
            generateAlchemistItems(inventorySize);
            break;
        case MarketType::CLOTHIER:
            generateClothierItems(inventorySize);
            break;
        case MarketType::JEWELER:
            generateJewelerItems(inventorySize);
            break;
        case MarketType::BOOKSTORE:
            generateBookstoreItems(inventorySize);
            break;
        case MarketType::MAGIC_SUPPLIES:
            generateMagicItems(inventorySize);
            break;
        case MarketType::FOOD:
            generateFoodItems(inventorySize);
            break;
        case MarketType::TAVERN:
            generateTavernItems(inventorySize);
            break;
        case MarketType::GENERAL:
        default:
            generateGeneralItems(inventorySize);
            break;
        }
    }

    // Helper functions to generate specific item types
    void generateBlacksmithItems(int count)
    {
        // Weapons
        addItemToInventory("iron_sword", "Iron Sword", "weapon", 100, randomInt(1, 3));
        addItemToInventory("steel_sword", "Steel Sword", "weapon", 250, randomInt(1, 2));
        addItemToInventory("iron_mace", "Iron Mace", "weapon", 80, randomInt(1, 2));
        addItemToInventory("iron_dagger", "Iron Dagger", "weapon", 40, randomInt(2, 4));
        addItemToInventory("hunting_bow", "Hunting Bow", "weapon", 120, randomInt(1, 2));

        // Armor
        addItemToInventory("iron_helmet", "Iron Helmet", "armor", 75, randomInt(1, 2));
        addItemToInventory("iron_cuirass", "Iron Cuirass", "armor", 150, randomInt(1, 2));
        addItemToInventory("leather_armor", "Leather Armor", "armor", 100, randomInt(1, 3));
        addItemToInventory("leather_boots", "Leather Boots", "armor", 50, randomInt(1, 3));
        addItemToInventory("iron_shield", "Iron Shield", "armor", 90, randomInt(1, 2));

        // Materials
        addItemToInventory("iron_ingot", "Iron Ingot", "metal", 20, randomInt(5, 15));
        addItemToInventory("steel_ingot", "Steel Ingot", "metal", 40, randomInt(3, 8));
        addItemToInventory("leather", "Leather", "material", 10, randomInt(5, 15));
        addItemToInventory("leather_strips", "Leather Strips", "material", 5, randomInt(10, 20));

        // Tools
        addItemToInventory("hammer", "Hammer", "tool", 30, randomInt(1, 3));
        addItemToInventory("tongs", "Tongs", "tool", 25, randomInt(1, 2));
        addItemToInventory("pickaxe", "Pickaxe", "tool", 50, randomInt(1, 2));
    }

    void generateAlchemistItems(int count)
    {
        // Potions
        addItemToInventory("minor_healing_potion", "Minor Healing Potion", "potion", 30, randomInt(3, 8));
        addItemToInventory("healing_potion", "Healing Potion", "potion", 60, randomInt(2, 5));
        addItemToInventory("minor_stamina_potion", "Minor Stamina Potion", "potion", 25, randomInt(3, 8));
        addItemToInventory("stamina_potion", "Stamina Potion", "potion", 50, randomInt(2, 5));
        addItemToInventory("minor_magicka_potion", "Minor Magicka Potion", "potion", 35, randomInt(3, 6));
        addItemToInventory("poison", "Poison", "potion", 45, randomInt(1, 4));

        // Ingredients
        addItemToInventory("red_herb", "Red Herb", "herb", 8, randomInt(5, 15));
        addItemToInventory("blue_flower", "Blue Flower", "herb", 12, randomInt(5, 12));
        addItemToInventory("mushroom", "Mushroom", "herb", 5, randomInt(8, 20));
        addItemToInventory("troll_fat", "Troll Fat", "ingredient", 20, randomInt(1, 5));
        addItemToInventory("void_salts", "Void Salts", "ingredient", 40, randomInt(1, 3));

        // Tools
        addItemToInventory("mortar_pestle", "Mortar and Pestle", "tool", 30, randomInt(1, 2));
        addItemToInventory("alembic", "Alembic", "tool", 45, randomInt(1, 2));
        addItemToInventory("retort", "Retort", "tool", 35, randomInt(1, 2));

        // Containers
        addItemToInventory("small_vial", "Small Vial", "container", 5, randomInt(5, 15));
        addItemToInventory("medium_bottle", "Medium Bottle", "container", 10, randomInt(3, 10));
    }

    void generateClothierItems(int count)
    {
        // Clothing
        addItemToInventory("common_shirt", "Common Shirt", "clothing", 15, randomInt(2, 5));
        addItemToInventory("fine_clothes", "Fine Clothes", "clothing", 80, randomInt(1, 3));
        addItemToInventory("noble_clothes", "Noble Clothes", "clothing", 150, randomInt(1, 2));
        addItemToInventory("common_pants", "Common Pants", "clothing", 15, randomInt(2, 5));
        addItemToInventory("fine_hat", "Fine Hat", "clothing", 45, randomInt(1, 3));
        addItemToInventory("cloak", "Cloak", "clothing", 30, randomInt(1, 4));
        addItemToInventory("gloves", "Gloves", "clothing", 20, randomInt(2, 4));

        // Materials
        addItemToInventory("linen", "Linen", "fabric", 10, randomInt(5, 15));
        addItemToInventory("cotton", "Cotton", "fabric", 15, randomInt(5, 12));
        addItemToInventory("silk", "Silk", "fabric", 35, randomInt(2, 8));
        addItemToInventory("thread", "Thread", "fabric", 5, randomInt(10, 20));

        // Tools
        addItemToInventory("needle", "Needle", "tool", 5, randomInt(2, 5));
        addItemToInventory("scissors", "Scissors", "tool", 15, randomInt(1, 3));
        addItemToInventory("measuring_tape", "Measuring Tape", "tool", 10, randomInt(1, 2));
    }

    void generateJewelerItems(int count)
    {
        // Jewelry
        addItemToInventory("silver_ring", "Silver Ring", "jewelry", 75, randomInt(1, 3));
        addItemToInventory("gold_ring", "Gold Ring", "jewelry", 150, randomInt(1, 2));
        addItemToInventory("silver_necklace", "Silver Necklace", "jewelry", 100, randomInt(1, 2));
        addItemToInventory("gold_necklace", "Gold Necklace", "jewelry", 200, randomInt(1, 2));
        addItemToInventory("sapphire_ring", "Sapphire Ring", "jewelry", 300, randomInt(0, 1));
        addItemToInventory("ruby_amulet", "Ruby Amulet", "jewelry", 350, randomInt(0, 1));

        // Gems
        addItemToInventory("amethyst", "Amethyst", "gem", 50, randomInt(1, 3));
        addItemToInventory("garnet", "Garnet", "gem", 60, randomInt(1, 3));
        addItemToInventory("sapphire", "Sapphire", "gem", 100, randomInt(0, 2));
        addItemToInventory("ruby", "Ruby", "gem", 120, randomInt(0, 2));
        addItemToInventory("emerald", "Emerald", "gem", 130, randomInt(0, 1));
        addItemToInventory("diamond", "Diamond", "gem", 250, randomInt(0, 1));

        // Materials
        addItemToInventory("silver_ingot", "Silver Ingot", "metal", 50, randomInt(1, 4));
        addItemToInventory("gold_ingot", "Gold Ingot", "metal", 100, randomInt(1, 3));

        // Tools
        addItemToInventory("jeweler_pliers", "Jeweler's Pliers", "tool", 20, randomInt(1, 2));
        addItemToInventory("magnifying_glass", "Magnifying Glass", "tool", 30, randomInt(1, 2));
    }

    void generateBookstoreItems(int count)
    {
        // Books
        addItemToInventory("history_book", "History of the Realm", "book", 35, randomInt(1, 3));
        addItemToInventory("fiction_book", "Tales of Adventure", "book", 25, randomInt(1, 4));
        addItemToInventory("skill_book_combat", "Sword and Shield", "book", 75, randomInt(0, 2));
        addItemToInventory("skill_book_magic", "Principles of Magicka", "book", 80, randomInt(0, 2));
        addItemToInventory("skill_book_stealth", "Shadow Tactics", "book", 75, randomInt(0, 2));

        // Scrolls
        addItemToInventory("scroll_healing", "Scroll of Healing", "scroll", 50, randomInt(1, 3));
        addItemToInventory("scroll_fireball", "Scroll of Fireball", "scroll", 65, randomInt(1, 2));
        addItemToInventory("scroll_shield", "Scroll of Shield", "scroll", 60, randomInt(1, 2));

        // Writing materials
        addItemToInventory("quill", "Quill", "tool", 10, randomInt(2, 5));
        addItemToInventory("ink_bottle", "Ink Bottle", "tool", 15, randomInt(2, 4));
        addItemToInventory("parchment", "Parchment", "material", 5, randomInt(5, 15));
        addItemToInventory("empty_journal", "Empty Journal", "book", 20, randomInt(1, 3));
    }

    void generateMagicItems(int count)
    {
        // Magic supplies
        addItemToInventory("soul_gem_petty", "Petty Soul Gem", "soul_gem", 50, randomInt(2, 5));
        addItemToInventory("soul_gem_lesser", "Lesser Soul Gem", "soul_gem", 100, randomInt(1, 3));
        addItemToInventory("soul_gem_common", "Common Soul Gem", "soul_gem", 150, randomInt(0, 2));
        addItemToInventory("soul_gem_greater", "Greater Soul Gem", "soul_gem", 300, randomInt(0, 1));

        // Staves
        addItemToInventory("staff_fire", "Staff of Fire", "staff", 250, randomInt(0, 1));
        addItemToInventory("staff_frost", "Staff of Frost", "staff", 250, randomInt(0, 1));
        addItemToInventory("staff_healing", "Staff of Healing", "staff", 300, randomInt(0, 1));

        // Magical ingredients
        addItemToInventory("void_salts", "Void Salts", "ingredient", 40, randomInt(1, 4));
        addItemToInventory("fire_salts", "Fire Salts", "ingredient", 45, randomInt(1, 3));
        addItemToInventory("frost_salts", "Frost Salts", "ingredient", 45, randomInt(1, 3));
        addItemToInventory("ectoplasm", "Ectoplasm", "ingredient", 25, randomInt(2, 4));

        // Spell tomes
        addItemToInventory("spell_tome_flames", "Spell Tome: Flames", "book", 150, randomInt(0, 1));
        addItemToInventory("spell_tome_healing", "Spell Tome: Healing", "book", 150, randomInt(0, 1));
        addItemToInventory("spell_tome_magelight", "Spell Tome: Magelight", "book", 120, randomInt(0, 1));
    }

    void generateFoodItems(int count)
    {
        // Basic foods
        addItemToInventory("bread", "Bread", "food", 5, randomInt(5, 15));
        addItemToInventory("cheese", "Cheese", "food", 8, randomInt(3, 10));
        addItemToInventory("apple", "Apple", "food", 3, randomInt(5, 15));
        addItemToInventory("potato", "Potato", "food", 2, randomInt(8, 20));
        addItemToInventory("carrot", "Carrot", "food", 2, randomInt(8, 20));
        addItemToInventory("cabbage", "Cabbage", "food", 3, randomInt(5, 12));

        // Meats
        addItemToInventory("beef", "Beef", "food", 10, randomInt(3, 8));
        addItemToInventory("venison", "Venison", "food", 12, randomInt(2, 6));
        addItemToInventory("chicken", "Chicken", "food", 8, randomInt(3, 8));
        addItemToInventory("fish", "Fish", "food", 7, randomInt(3, 10));

        // Prepared foods
        addItemToInventory("stew", "Stew", "food", 15, randomInt(2, 5));
        addItemToInventory("pie", "Pie", "food", 12, randomInt(2, 5));
        addItemToInventory("sweetroll", "Sweetroll", "food", 5, randomInt(3, 8));

        // Ingredients
        addItemToInventory("salt", "Salt", "ingredient", 5, randomInt(3, 8));
        addItemToInventory("flour", "Flour", "ingredient", 3, randomInt(5, 10));
        addItemToInventory("sugar", "Sugar", "ingredient", 6, randomInt(3, 8));
        addItemToInventory("spices", "Spices", "ingredient", 12, randomInt(2, 5));
    }
...
void
generateTavernItems(int count)
{
    // Drinks
    addItemToInventory("ale", "Ale", "drink", 8, randomInt(8, 20));
    addItemToInventory("wine", "Wine", "drink", 15, randomInt(5, 12));
    addItemToInventory("mead", "Mead", "drink", 10, randomInt(5, 15));
    addItemToInventory("brandy", "Brandy", "drink", 20, randomInt(3, 8));

    // Food
    addItemToInventory("bread", "Bread", "food", 5, randomInt(3, 10));
    addItemToInventory("cheese", "Cheese", "food", 8, randomInt(3, 8));
    addItemToInventory("stew", "Stew", "food", 15, randomInt(3, 8));
    addItemToInventory("roasted_meat", "Roasted Meat", "food", 12, randomInt(3, 6));
    addItemToInventory("pie", "Pie", "food", 12, randomInt(2, 5));

    // Entertainment items
    addItemToInventory("dice", "Dice", "entertainment", 5, randomInt(1, 3));
    addItemToInventory("playing_cards", "Playing Cards", "entertainment", 8, randomInt(1, 2));
}

void generateGeneralItems(int count)
{
    // A mix of various basic items
    // Food
    addItemToInventory("bread", "Bread", "food", 5, randomInt(3, 8));
    addItemToInventory("cheese", "Cheese", "food", 8, randomInt(2, 5));
    addItemToInventory("apple", "Apple", "food", 3, randomInt(3, 10));

    // Tools
    addItemToInventory("hammer", "Hammer", "tool", 30, randomInt(1, 3));
    addItemToInventory("saw", "Saw", "tool", 25, randomInt(1, 2));
    addItemToInventory("rope", "Rope", "tool", 15, randomInt(2, 5));
    addItemToInventory("fishing_pole", "Fishing Pole", "tool", 20, randomInt(1, 3));

    // Materials
    addItemToInventory("leather", "Leather", "material", 10, randomInt(3, 8));
    addItemToInventory("iron_ingot", "Iron Ingot", "metal", 20, randomInt(2, 5));
    addItemToInventory("linen", "Linen", "fabric", 10, randomInt(3, 8));

    // Misc
    addItemToInventory("torch", "Torch", "tool", 5, randomInt(3, 10));
    addItemToInventory("lantern", "Lantern", "tool", 15, randomInt(1, 3));
    addItemToInventory("waterskin", "Waterskin", "container", 8, randomInt(2, 5));
    addItemToInventory("backpack", "Backpack", "container", 25, randomInt(1, 2));

    // Basic weapons and armor
    addItemToInventory("iron_dagger", "Iron Dagger", "weapon", 40, randomInt(1, 3));
    addItemToInventory("leather_boots", "Leather Boots", "armor", 50, randomInt(1, 2));
}
...
struct Property {
    ...
    struct PropertyUpgrade {
        ...
        int cost;
        bool installed;
        std::function<void(Property&)> applyEffect;
    };
...
int main()
{
    ...
    // Create some markets
    Market* generalMarket = economicSystem->createMarket("general_whiterun", "Whiterun General Goods", MarketType::GENERAL);
    generalMarket->region = "Whiterun";
    generalMarket->wealthLevel = 1.2f;
    generalMarket->ownerName = "Belethor";

    Market* blacksmithMarket = economicSystem->createMarket("blacksmith_whiterun", "Warmaiden's", MarketType::BLACKSMITH);
    blacksmithMarket->region = "Whiterun";
    blacksmithMarket->wealthLevel = 1.0f;
    blacksmithMarket->ownerName = "Ulfberth War-Bear";

    Market* alchemistMarket = economicSystem->createMarket("alchemist_whiterun", "Arcadia's Cauldron", MarketType::ALCHEMIST);
    alchemistMarket->region = "Whiterun";
    alchemistMarket->wealthLevel = 1.1f;
    alchemistMarket->ownerName = "Arcadia";

    Market* tavernMarket = economicSystem->createMarket("tavern_whiterun", "The Bannered Mare", MarketType::TAVERN);
    tavernMarket->region = "Whiterun";
    tavernMarket->wealthLevel = 1.3f;
    tavernMarket->ownerName = "Hulda";

    // Create some commodities
    TradeCommodity ironCommodity;
    ironCommodity.id = "iron";
    ironCommodity.name = "Iron";
    ironCommodity.basePrice = 20.0f;
    ironCommodity.currentPrice = 20.0f;
    ironCommodity.supply = 50;
    ironCommodity.demand = 40;
    ironCommodity.baseSupply = 50;
    ironCommodity.baseDemand = 40;
    ironCommodity.origin = "Whiterun";
    ironCommodity.isLuxury = false;
    ironCommodity.volatility = 0.1f;

    TradeCommodity leatherCommodity;
    leatherCommodity.id = "leather";
    leatherCommodity.name = "Leather";
    leatherCommodity.basePrice = 10.0f;
    leatherCommodity.currentPrice = 10.0f;
    leatherCommodity.supply = 60;
    leatherCommodity.demand = 50;
    leatherCommodity.baseSupply = 60;
    leatherCommodity.baseDemand = 50;
    leatherCommodity.origin = "Whiterun";
    leatherCommodity.isLuxury = false;
    leatherCommodity.volatility = 0.15f;

    TradeCommodity potionCommodity;
    potionCommodity.id = "healing_potion";
    potionCommodity.name = "Healing Potion";
    potionCommodity.basePrice = 50.0f;
    potionCommodity.currentPrice = 50.0f;
    potionCommodity.supply = 20;
    potionCommodity.demand = 30;
    potionCommodity.baseSupply = 20;
    potionCommodity.baseDemand = 30;
    potionCommodity.origin = "Winterhold";
    potionCommodity.isLuxury = true;
    potionCommodity.volatility = 0.2f;

    TradeCommodity foodCommodity;
    foodCommodity.id = "food";
    foodCommodity.name = "Food";
    foodCommodity.basePrice = 8.0f;
    foodCommodity.currentPrice = 8.0f;
    foodCommodity.supply = 80;
    foodCommodity.demand = 100;
    foodCommodity.baseSupply = 80;
    foodCommodity.baseDemand = 100;
    foodCommodity.origin = "Whiterun";
    foodCommodity.isLuxury = false;
    foodCommodity.volatility = 0.3f;

    // Add commodities to markets
    blacksmithMarket->addCommodity(ironCommodity);
    generalMarket->addCommodity(leatherCommodity);
    alchemistMarket->addCommodity(potionCommodity);
    tavernMarket->addCommodity(foodCommodity);
    generalMarket->addCommodity(foodCommodity);

    // Create trade routes
    economicSystem->addTradeRoute("whiterun_winterhold", "Whiterun-Winterhold Road",
        "blacksmith_whiterun", "alchemist_whiterun", 100.0f, 0.2f);

    // Create economic events
    EconomicEvent warEvent;
    warEvent.id = "civil_war";
    warEvent.name = "Civil War Intensifies";
    warEvent.description = "Fighting has increased between Imperial and Stormcloak forces.";
    warEvent.commoditySupplyEffects["iron"] = 0.7f; // Iron supply drops to 70%
    warEvent.commodityDemandEffects["iron"] = 1.5f; // Iron demand increases by 50%
    warEvent.commoditySupplyEffects["food"] = 0.8f; // Food supply drops to 80%
    warEvent.tradeRouteDisruptions["whiterun_winterhold"] = true; // Route disrupted
    warEvent.affectedRegions = { "Whiterun", "Winterhold" };
    warEvent.duration = 20; // 20 days
    warEvent.daysSinceStart = 0;

    EconomicEvent harvestEvent;
    harvestEvent.id = "good_harvest";
    harvestEvent.name = "Bountiful Harvest";
    harvestEvent.description = "This year's harvest has been particularly good.";
    harvestEvent.commoditySupplyEffects["food"] = 1.5f; // Food supply increases by 50%
    harvestEvent.commodityDemandEffects["food"] = 0.9f; // Food demand slightly decreases
    harvestEvent.affectedRegions = { "Whiterun", "Riften" };
    harvestEvent.duration = 30; // 30 days
    harvestEvent.daysSinceStart = 0;

    // Add events to potential events
    economicSystem->addPotentialEvent(warEvent);
    economicSystem->addPotentialEvent(harvestEvent);
    ...
    // Create some investment opportunities
    BusinessInvestment tavernInvestment;
    tavernInvestment.id = "bannered_mare";
    tavernInvestment.name = "The Bannered Mare";
    tavernInvestment.description = "A popular tavern in Whiterun with steady clientele.";
    tavernInvestment.marketId = "tavern_whiterun";
    tavernInvestment.initialCost = 500;
    tavernInvestment.riskLevel = 0.2f;
    tavernInvestment.returnRate = 0.08f;
    tavernInvestment.marketSensitivity = 0.8f;
    tavernInvestment.payoutInterval = 7;

    BusinessInvestment mineInvestment;
    mineInvestment.id = "iron_mine";
    mineInvestment.name = "Iron-Breaker Mine";
    mineInvestment.description = "An iron mine with good production but some risk.";
    mineInvestment.marketId = "blacksmith_whiterun";
    mineInvestment.initialCost = 800;
    mineInvestment.riskLevel = 0.5f;
    mineInvestment.returnRate = 0.12f;
    mineInvestment.marketSensitivity = 1.3f;
    mineInvestment.payoutInterval = 10;

    BusinessInvestment alchemyInvestment;
    alchemyInvestment.id = "arcadias_cauldron";
    alchemyInvestment.name = "Arcadia's Cauldron";
    alchemyInvestment.description = "An alchemy shop with specialty products.";
    alchemyInvestment.marketId = "alchemist_whiterun";
    alchemyInvestment.initialCost = 700;
    alchemyInvestment.riskLevel = 0.4f;
    alchemyInvestment.returnRate = 0.10f;
    alchemyInvestment.marketSensitivity = 1.0f;
    alchemyInvestment.payoutInterval = 7;

    // Add investments
    investmentSystem->addInvestmentOpportunity(tavernInvestment);
    investmentSystem->addInvestmentOpportunity(mineInvestment);
    investmentSystem->addInvestmentOpportunity(alchemyInvestment);
    ...
    houseProperty.id = "breezehome";
    houseProperty.name = "Breezehome";
    houseProperty.description = "A cozy house in Whiterun with a great view.";
    houseProperty.regionId = "Whiterun";
    houseProperty.type = PropertyType::HOUSE;
    houseProperty.purchasePrice = 5000;
    houseProperty.weeklyIncome = 0;
    houseProperty.weeklyUpkeep = 50;
    houseProperty.storageCapacity = 200;

    // Add upgrades to house
    Property::PropertyUpgrade kitchenUpgrade;
    kitchenUpgrade.id = "kitchen";
    kitchenUpgrade.name = "Kitchen";
    kitchenUpgrade.description = "A fully equipped kitchen for cooking.";
    kitchenUpgrade.cost = 500;
    kitchenUpgrade.installed = false;
    kitchenUpgrade.applyEffect = [](Property& property) {
        property.weeklyUpkeep += 5;
    };

    Property::PropertyUpgrade alchemyLabUpgrade;
    alchemyLabUpgrade.id = "alchemy_lab";
    alchemyLabUpgrade.name = "Alchemy Laboratory";
    alchemyLabUpgrade.description = "An alchemy lab for crafting potions.";
    alchemyLabUpgrade.cost = 600;
    alchemyLabUpgrade.installed = false;
    alchemyLabUpgrade.applyEffect = [](Property& property) {
        property.weeklyUpkeep += 10;
    };

    // Add upgrades to property
    houseProperty.addUpgrade(kitchenUpgrade);
    houseProperty.addUpgrade(alchemyLabUpgrade);

    Property shopProperty;
    shopProperty.id = "warmaiden_shop";
    shopProperty.name = "Warmaiden's Shop";
    shopProperty.description = "A blacksmith shop in Whiterun, currently for sale.";
    shopProperty.regionId = "Whiterun";
    shopProperty.type = PropertyType::SHOP;
    shopProperty.purchasePrice = 10000;
    shopProperty.weeklyIncome = 300;
    shopProperty.weeklyUpkeep = 100;
    shopProperty.storageCapacity = 500;

    // Add tenant to shop
    Property::Tenant shopkeeper;
    shopkeeper.id = "ulfberth";
    shopkeeper.name = "Ulfberth War-Bear";
    shopkeeper.rentAmount = 250;
    shopkeeper.daysSinceLastPayment = 0;
    shopkeeper.paymentInterval = 7;
    shopkeeper.reliability = 0.9f;

    shopProperty.addTenant(shopkeeper);

    propertySystem->addProperty(houseProperty);
    propertySystem->addProperty(shopProperty);
