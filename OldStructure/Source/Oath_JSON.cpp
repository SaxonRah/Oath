...
int main()
{
    ...
    if (controller.currentNodes["WorldSystem"]->nodeName == "VillageCenter") {
        LocationNode* villageCenterLocation = dynamic_cast<LocationNode*>(controller.currentNodes["WorldSystem"]);
        if (villageCenterLocation) {
    ...
    for (const auto& [systemName, node] : controller.currentNodes) {
        std::cout << systemName << " current node ID: " << node->nodeID.persistentID << std::endl;
    }
